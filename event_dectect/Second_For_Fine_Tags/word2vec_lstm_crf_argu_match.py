...
from event_dectect.Second_For_Fine_Tags import input_fn as INPUT,model_fn as MODEL, config_center as NEW_CONFIG
...
def main(FLAGS,sentencs_words_firstTags_list=None,words_firstTags_indxPairs_sentences_speaker=None,output_path = None):
    print(FLAGS)
    ...
    if FLAGS.ifTrain :
        print('获取训练数据。。。')
        train_inpf = functools.partial(INPUT.input_fn, input_dir=FLAGS.labeled_data_path+'_for_second',dirs=FLAGS.train_folder,
        ...
        print('获取评估数据。。。')
        eval_inpf = functools.partial(INPUT.input_fn, input_dir=FLAGS.labeled_data_path+'_for_second',dirs=FLAGS.dev_folder,
    ...
    if FLAGS.ifTest:
        print('获取预测数据。。。')
        test_inpf = functools.partial(INPUT.input_fn, input_dir=FLAGS.labeled_data_path+'_for_second',dirs=FLAGS.test_folder,
                                      shuffe=False, num_epochs=FLAGS.num_epochs, batch_size=FLAGS.batch_size,max_sequence_length=FLAGS.max_sequence_length)

        predictions = estimator.predict(input_fn=test_inpf)
        pred_true = INPUT.generator_fn(input_dir=FLAGS.labeled_data_path+'_for_second',dirs=FLAGS.test_folder,max_sequence_length = FLAGS.max_sequence_length,noEmbedding=True)
    ...
    if(FLAGS.ifPredict and words_firstTags_indxPairs_sentences_speaker):
        '''根据原句分词和第一个模型的预测标记序列 让第二个模型预测并抽取事实'''
        ...
        def handlerOneInput(words, first_tags, index_pairs, sentence,speaker):
            ...
            return results, index_pairs_list, sentences,speakers

        words_list, first_tags_list, index_pairs_list, sentences,speakers = words_firstTags_indxPairs_sentences_speaker
        # 构造第二个模型的输入list
        # 针对第一个模型的预测结果，针对每个触发词都会构成一条新的预测example,也就是第一个模型的单条结果可能会生成第二个模型的多条预测输入
        sentence_words_firstTags_trueTriggerTags = []
        new_index_pairs_list = []
        new_sentences = []
        new_speakers = []
        for words, first_tags, index_pairs, sentence,speaker in zip(words_list, first_tags_list, index_pairs_list, sentences,speakers):
            the_words_firstTags_newTags_list, the_index_pairs_list, the_sentences,the_speakers = handlerOneInput(words, first_tags,
                                                                                                    index_pairs,
                                                                                                    sentence,speaker)
            sentence_words_firstTags_trueTriggerTags.extend(the_words_firstTags_newTags_list)
            new_index_pairs_list.extend(the_index_pairs_list)
            new_sentences.extend(the_sentences)
            new_speakers.extend(the_speakers)
        pred_inpf = functools.partial(INPUT.input_fn, input_dir=None, shuffe=False, num_epochs=FLAGS.num_epochs,
                                      batch_size=FLAGS.batch_size, max_sequence_length=FLAGS.max_sequence_length,
                                      sentence_words_firstTags_trueTriggerTags=sentence_words_firstTags_trueTriggerTags)
        predictions = estimator.predict(input_fn=pred_inpf)
        ...
        for words, tags in zip(new_words_list, new_tags_list):
            ...
            print(' '.join(tags))
            print('\n')
        return new_words_list, new_tags_list, new_index_pairs_list, new_sentences,new_speakers
